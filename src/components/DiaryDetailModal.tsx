      ...
      {isOpen && record && (
        ...
        <div className="p-6 overflow-y-auto space-y-5 text-xs font-semibold scrollbar-thin flex-1">
          ...
          <div className="space-y-2">
            <h4 className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest border-b pb-1 border-slate-200 dark:border-slate-800 flex items-center gap-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50/50 dark:bg-slate-900/40 p-3 rounded-lg border border-slate-100 dark:border-slate-800">
              <div>
                <span className="block text-[9px] text-slate-400 uppercase font-black">Multiplicador Solicitante</span>
                <span className="text-slate-800 dark:text-slate-200 font-black">{record.solicitante}</span>
              ...
              <div>
                <span className="block text-[9px] text-slate-400 uppercase font-black">Data de Ocorrência</span>
                <span className="text-slate-800 dark:text-slate-300 font-black flex items-center gap-1">
                  <Calendar className="w-3.5 h-3.5 text-slate-400" /> {record.dataOcorrencia}
                </span>
              </div>
              <div className="sm:col-span-2">
                <span className="block text-[9px] text-slate-400 uppercase font-black">Unidade de Ensino Atendida</span>
                <span className="text-slate-800 dark:text-slate-200 font-black">{record.escolaNome}</span>
                {record.escolaInep && (
                  <span className="block text-[10px] text-slate-400 font-mono font-medium">Código INEP: {record.escolaInep}</span>
                )}
              </div>
              <div>
                <span className="block text-[9px] text-slate-400 uppercase font-black">UTEC de Apoio / Responsável</span>
                <span className="text-slate-800 dark:text-slate-200 font-bold">{record.utecName}</span>
              </div>
              <div>
                <span className="block text-[9px] text-slate-400 uppercase font-black">Categoria de Atendimento</span>
          ...
          {/* Atividade */}
          <div className="space-y-2">
            <h4 className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest border-b pb-1 border-slate-200 dark:border-slate-800 flex items-center gap-1">
              <BookOpen className="w-3.5 h-3.5" /> Detalhamento da Atividade
            </h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50/50 dark:bg-slate-900/40 p-3 rounded-lg border border-slate-100 dark:border-slate-800">
              {record.temaDaAtividade && (
                <div className="sm:col-span-2">
                  <span className="block text-[9px] text-slate-400 uppercase font-black">Tema da Atividade</span>
                  <span className="text-slate-800 dark:text-slate-200 font-bold">{record.temaDaAtividade}</span>
                </div>
              )}
              <div>
                <span className="block text-[9px] text-slate-400 uppercase font-black">Turnos de Atendimento</span>
                <span className="text-slate-800 dark:text-slate-200 font-bold">
                  {[record.turno1, record.turno2, record.turno3].filter(Boolean).join(', ') || 'Não especificado'}
                </span>
              </div>
              <div>
                <span className="block text-[9px] text-slate-400 uppercase font-black">Local</span>
              ...
              {record.atividadesDesenvolvidas && (
                <div className="sm:col-span-2">
                  <span className="block text-[9px] text-slate-400 uppercase font-black">Atividades Desenvolvidas</span>
          ...
          {/* Impacto */}
          <div className="space-y-2">
            <h4 className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest border-b pb-1 border-slate-200 dark:border-slate-800 flex items-center gap-1">
            ...
            <div className="grid grid-cols-2 gap-4 bg-slate-50/50 dark:bg-slate-900/40 p-3 rounded-lg border border-slate-100 dark:border-slate-800">
              <div>
                <span className="block text-[9px] text-slate-400 uppercase font-black">Professores Impactados</span>
                <span className="text-slate-800 dark:text-slate-100 font-black text-sm">{record.qtdProfessores}</span>
              ...
              <div>
                <span className="block text-[9px] text-slate-400 uppercase font-black">Estudantes Impactados</span>
                <span className="text-slate-800 dark:text-slate-100 font-black text-sm">{record.qtdEstudantes}</span>
              ...
              {record.grupoImpacto && (
                <div className="col-span-2">
                  <span className="block text-[9px] text-slate-400 uppercase font-black">Grupo Impactado</span>
          ...
          {(record.redsFisicos || record.softwares) && (
            <div className="space-y-2">
              <h4 className="text-[10px] font-black text-amber-600 dark:text-amber-450 uppercase tracking-widest border-b pb-1 border-slate-200 dark:border-slate-800 flex items-center gap-1">
                <PenTool className="w-3.5 h-3.5" /> Recursos & Tecnologias
              </h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50/50 dark:bg-slate-900/40 p-3 rounded-lg border border-slate-100 dark:border-slate-800">
                {record.redsFisicos && (
                  <div>
                    <span className="block text-[9px] text-slate-400 uppercase font-black">Recursos Físicos / Hardware</span>
                    <span className="text-slate-800 dark:text-slate-200 font-bold">{record.redsFisicos}</span>
                  </div>
                )}
                {record.softwares && (
                  <div>
                    <span className="block text-[9px] text-slate-400 uppercase font-black">Mídias & Softwares</span>
          ...
          {(record.observacao || record.observacoes || record.ocorrencia) && (
            <div className="space-y-2">
              <h4 className="text-[10px] font-black text-rose-600 dark:text-rose-400 uppercase tracking-widest border-b pb-1 border-slate-200 dark:border-slate-800 flex items-center gap-1">
              ...
              <div className="bg-slate-50/50 dark:bg-slate-900/40 p-3 rounded-lg border border-slate-100 dark:border-slate-800 space-y-2.5">
                {(record.observacao || record.observacoes) && (
                  <div>
                    <span className="block text-[9px] text-slate-400 uppercase font-black">Observação Técnica</span>
                ...
                {record.ocorrencia && (
                  <div>
                    <span className="block text-[9px] text-rose-500 uppercase font-black">Descrição da Ocorrência</span>
        ...
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-[10px] font-bold text-slate-400">
          ...
          <button
            onClick={onClose}
            className="px-4 py-2 bg-slate-800 hover:bg-slate-900 dark:bg-slate-700 dark:hover:bg-slate-600 text-white rounded-lg font-black tracking-wide uppercase transition-all cursor-pointer"
          >
            Fechar Ficha
          </button>
        </div>
